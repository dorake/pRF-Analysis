...
def get_data_source_dict(data, condition, colors_val = None, x_gain = 0, y_shift = 0):
    ...
    if (condition == 'map') or (condition == 'ecc') or (condition == 'ecc_gainRatio'):
        data_source                     =   {'x':            data[:,0],                                # x coordinate
                                            'y':             data[:,1],                                # y coordinate
                                            'sigma':         data[:,2],                                # size (sigma)
                                            'beta':          data[:,3],                                # amplitude (beta)
                                            'baseline':      data[:,4],                                # baseline
                                            'rsq':           data[:,5],                                # residual square root
                                            'cv_rsq':        data[:,6],                                # cross-validated residual square root
                                            'stim_ratio':    data[:,7]*100,                            # ratio of pRF in stim
                                            'ecc':           data[:,8],
                                            'x_gain':        data[:,9],
                                            'amp_gain':      data[:,10],
